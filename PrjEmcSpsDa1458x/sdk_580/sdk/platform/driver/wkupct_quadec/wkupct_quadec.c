...
extern uint32_t rcx_freq; // RCX calibrated frequency value stored in retention memory by calibration procedure
...
static uint16_t wkupct_deb_time                 __attribute__((section("retention_mem_area0"), zero_init)); // Wakeup timer debouncing time
...
{
    uint8_t temp;
    wkupct_deb_time = deb_time;                                     // Store value in retention memory
...
void wkupct_tweak_deb_time(bool tweak)
{
    if (arch_clk_is_RCX20())
    {
        if (tweak)
        {
            ...
            uint32_t temp = (rcx_freq * wkupct_deb_time) / 32768;
            SetBits16(WKUP_CTRL_REG, WKUP_DEB_VALUE, temp & 0x3F);
        }
        else
        {
            SetBits16(WKUP_CTRL_REG, WKUP_DEB_VALUE, wkupct_deb_time & 0x3F);
        }
    }
}

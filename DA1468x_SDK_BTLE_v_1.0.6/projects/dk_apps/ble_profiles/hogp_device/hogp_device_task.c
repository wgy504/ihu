...
PRIVILEGED_DATA static queue_t notification_queue;
                                                                                ...
                                                                                bool append)
{
        ...
        if (report_type == REPORT_TYPE_MOUSE) {
                if (protocol_mode == HIDS_PROTOCOL_MODE_REPORT) {
                        status = hids_set_report_value(hids, HIDS_REPORT_TYPE_INPUT, MOUSE_REPORT_ID, length, data);
                } else {
                        /* Boot report for mouse contains 3 bytes: buttons, x, y */
                        status = hids_set_boot_mouse_input_value(hids, 3, data);
                }
        } else {
                if (protocol_mode == HIDS_PROTOCOL_MODE_REPORT) {
                        status = hids_set_report_value(hids, HIDS_REPORT_TYPE_INPUT, KEYBOARD_REPORT_ID, length, data);
                } else {
                        status = hids_set_boot_keyboard_input_value(hids, length, data);
                }
        }
        ...
        if (!status && append) {
                ...
                if (queue_length(&notification_queue) > MAX_PENDING_REPORTS) {
                        goto done;
                }

                notification = OS_MALLOC(sizeof(*notification) + length);
                notification->length = length;
                notification->report_type = report_type;
                memcpy(notification->data, data, length);

                queue_push_back(&notification_queue, notification);
        }
done:
        return status;
}
...
static void update_coords(void)
{
        ...
        send_report(sizeof(report), (uint8_t *) report, REPORT_TYPE_MOUSE, true);
}
...
static void update_keyboard_buttons(void)
{
        ...
        send_report(sizeof(report), report, REPORT_TYPE_KEYBOARD, true);
        /* Clear buttons */
        memset(report, 0, 8);
        send_report(sizeof(report), report, REPORT_TYPE_KEYBOARD, true);
}
...
static void control_point_cb(ble_service_t *svc, hids_cp_command_t command)
{
        ...
        if (cp_state == HIDS_CONTROL_POINT_EXIT_SUSPEND) {
                OS_TIMER_START(hid_timer, MOUSE_TIMEOUT / OS_PERIOD_MS);
        } else {
                OS_TIMER_STOP(hid_timer, MOUSE_TIMEOUT / OS_PERIOD_MS);
        }
}

static void report_sent_cb(ble_service_t *svc)
{
        notification_t *notification;
        bool status;

        do {
                notification = queue_peek_front(&notification_queue);
                if (!notification) {
                        break;
                }

                status = send_report(notification->length, notification->data,
                                                                notification->report_type, false);

                /* Remove notification if set value was successful */
                if (status) {
                        queue_pop_front(&notification_queue);
                        OS_FREE(notification);
                }
        } while (status);
}
...
void hogp_device_task(void *params)
{
        ...
        ble_service_add(hids_includes[0]);
        ...
        queue_init(&notification_queue);

#if CFG_DEVICE_MOUSE
        /* Create and start timer for updating coords */

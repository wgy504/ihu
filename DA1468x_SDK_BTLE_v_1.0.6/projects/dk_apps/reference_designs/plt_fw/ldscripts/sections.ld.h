...
SECTIONS
{
        .text :
        {
                ...
                *(EXCLUDE_FILE(*libgcc.a:_aeabi_uldivmod.o *libgcc.a:_muldi3.o \
                               *libgcc.a:_dvmd_tls.o *libgcc.a:bpabi.o *libgcc.a:_udivdi3.o \
                               *libgcc.a:_clzdi2.o *libgcc.a:_clzsi2.o) .text*)
         ...
         * Put any code that has to be retained during pm_mode_extended_sleep or pm_mode_deep_sleep
	...
	ASSERT(0, "Unsupported chip version")
        ...
        __mirrored_image_size = __etext + (__data_end__ - __data_start__) - ORIGIN(ROM);
                ...
                && (dg_configBLACK_ORCA_IC_STEP <= BLACK_ORCA_IC_STEP_D))
        /* Check if RAM does not overlap ROM variables */
        ASSERT( !( ((ORIGIN(RAM) >= 0x7FDEC00) && (ORIGIN(RAM) < 0x07FE0000)) \
                || ((ORIGIN(RAM) < 0x7FDEC00) && ((ORIGIN(RAM) + LENGTH(RAM)) > 0x7FDEC00)) ), \
                "ROM variables region (starting at 0x7FDEC00) overflowed by RAM")

        /* Check if RetRAM0 does not overlap ROM variables */
        ASSERT(__RetRAM0_data_end__ < 0x7FDEC00, \
                "ROM variables region (starting at 0x7FDEC00) overflowed by RetRAM0")
        ...
        INCLUDE da14681_00_rom.symbols
                ...
                && (dg_configBLACK_ORCA_IC_STEP == BLACK_ORCA_IC_STEP_E))
        /* Check if RAM does not overlap ROM variables */
        ASSERT( !( ((ORIGIN(RAM) >= 0x07FDC000) && (ORIGIN(RAM) < 0x07FE0000)) \
                || ((ORIGIN(RAM) < 0x07FDC000) && ((ORIGIN(RAM) + LENGTH(RAM)) > 0x07FDC000)) ), \
                "ROM variables region (starting at 0x07FDC000) overflowed by RAM")

        /* Check if RetRAM0 does not overlap ROM variables */
        ASSERT(__RetRAM0_data_end__ < 0x7FDC000, \

...
{
        if (status != ATT_ERROR_OK) {
                printf("ERROR: failed to configure device (write MEM_DEV failed with code=0x%02x)\r\n",
                                                                                        status);
                return;
        }
...
{
        if (status != ATT_ERROR_OK) {
                printf("ERROR: Failed to set new patch block length\r\n");
                return;
        }
...
static void handle_evt_gap_connected(ble_evt_gap_connected_t *evt)
{
        ...
        ble_gattc_get_mtu(evt->conn_idx, &peer_info.mtu);

...
void task_i2c_async_get_temp_func(const struct task_item *task)
{
        ...
        if (++cnt > 1000) {
                uart_printfln_s("current temperature: %d.%04d C", t_out, fract);
                cnt = 0;
        }
...
void menu_i2c_async_write_to_eeprom_func(const struct menu_item *m, bool checked)
{
        ...
        if (!i2c_signal.error) {
                 ...
                 */
                uart_printfln_s("write to EEPROM:  failed %x", i2c_signal.error);
        }
...
void menu_i2c_async_read_from_eeprom_func(const struct menu_item *m, bool checked)
{
         ...
         */
        if (i2c_signal.error) {
                uart_printfln_s("read from EEPROM:  failed %x", i2c_signal.error);
        } else {
                uart_printfln_s("read from EEPROM:  %s", read_buffer);
        }
}

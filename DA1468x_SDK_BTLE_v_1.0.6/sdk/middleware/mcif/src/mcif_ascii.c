...
PRIVILEGED_DATA static uint8_t framebuffer[MCIF_ASCII_MAX_LINE + 1];
PRIVILEGED_DATA static int pos;
        ...
        struct mcif_message_s **rxmsg)
{
        ...
        case '\n':
                ...
                *rxmsg = OS_MALLOC(sizeof(struct mcif_message_s) + pos);
                ...
                break;
        default:
                if (pos == MCIF_ASCII_MAX_LINE) {
                        /* No more space in the buffer. Ignore the char */
                        break;
                }
                hw_uart_send(MCIF_UART, &rxbyte, 1, NULL, NULL);
                framebuffer[pos++] = rxbyte;
                framebuffer[pos] = '\0';
        }

...
#if SUOTA_PSM
PRIVILEGED_DATA static uint16_t l2cap_max_credits;
PRIVILEGED_DATA static uint16_t l2cap_watermark;
#endif
...
static bool suota_state_w4_header(suota_service_t *suota)
{
        ...
        if (sizeof(suota->header) + suota->header.code_size > SUOTA_MAX_IMAGE_SIZE) {
...
static void update_l2cap_parameters(void)
{
        uint16_t mtu;
        uint16_t mps = 23;
        uint16_t sdu_per_buf; /* number of SDUs of max length that will fit in SUOTA buffer */
        uint16_t credits_per_sdu; /* number of credits consumed by SDU of max length */

        ble_gap_mtu_size_get(&mtu);

        sdu_per_buf = CEIL_DIV(SUOTA_BUFFER_SIZE , mtu - 2);

        credits_per_sdu = CEIL_DIV(mtu, mps);

        /*
         * We give credits for all SDUs that can fit in SUOTA buffer and one extra - the one that
         * can start being received while we give back credits at "full" buffer. This minimizes
         * number of credits operations over the air.
         */
        l2cap_max_credits = (sdu_per_buf + 1) * credits_per_sdu;
        l2cap_watermark = sdu_per_buf * credits_per_sdu;
}
...
#if SUOTA_PSM
                update_l2cap_parameters();

                /*
                 * Client wrote data length, now we can start listening on PSM since data transfer will
                 * begin in a moment.
                 */
                ble_l2cap_listen(conn_idx, SUOTA_PSM, GAP_SEC_LEVEL_1, l2cap_max_credits,
                                                                                &suota->l2cap_scid);
...
static void l2cap_data_ind(suota_service_t *suota, const ble_evt_l2cap_data_ind_t *evt)
{
        ...
        if (!handle_patch_data(suota, evt->data, evt->length)) {
                ...
                if (suota->pending_credits >= l2cap_watermark) {
...
#if SUOTA_PSM
        l2cap_psm_h += suota->svc.start_h;
        ble_gatts_set_value(l2cap_psm_h, sizeof(psm), &psm);
#endif

...
static void send_weight_indication(ws_service_t *wss, uint16_t conn_idx,
                                                const wss_weight_measurement_t *measurement)
{
        uint8_t value[15];
        uint16_t len = pack_measurement(wss, measurement, value);

        ble_gatts_send_event(conn_idx, wss->measurement_h, GATT_EVENT_INDICATION, len, value);
}

void wss_indicate_weight(ble_service_t *svc, uint16_t conn_idx,
                                                const wss_weight_measurement_t *measurement)
{
        ws_service_t *wss = (ws_service_t *) svc;
        ...
        send_weight_indication(wss, conn_idx, measurement);
}

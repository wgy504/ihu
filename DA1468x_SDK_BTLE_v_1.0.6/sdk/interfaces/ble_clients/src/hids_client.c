                                                        ...
                                                        const ble_evt_gattc_write_completed_t *evt)
{
        ...
        if (handle == hids_client->boot_keyboard_input_ccc_h) {
                ...
                if (hids_client->cb->input_report_set_notif_state) {
                        hids_client->cb->input_report_set_notif_state(client, report->report_id,
                                                                                        status);
                }
        }
}

static void register_hids_client(hids_client_t *hids_client)
{
        uint8_t protocol_mode = hids_client->mode;

        /* Write protocol mode */
        if (hids_client->protocol_mode_val_h) {
                ble_gattc_write_no_resp(hids_client->client.conn_idx,
...
static void attach(ble_client_t *client)
{
        hids_client_t *hids_client = (hids_client_t *) client;

        register_hids_client(hids_client);
}
                                                        ...
                                                        const ble_evt_gattc_browse_svc_t *evt)
{
        ...
        hids_client->client.attach = attach;

...
extern void patch_gapc_task(void);
...
#if ((dg_configBLE_ADV_STOP_DELAY_ENABLE == 1) || (RWBLE_SW_VERSION_MAJOR < 8))
                if (msgBuf->msg.gtl.msg_id == GAPM_CMP_EVT) {
                        struct gapm_cmp_evt *evt = (struct gapm_cmp_evt *) msgBuf->msg.gtl.param;
#if (RWBLE_SW_VERSION_MAJOR < 8)
                        if ((evt->operation == GAPM_RESET) && (evt->status == GAP_ERR_NO_ERROR)) {
                                /* Patch task handlers */
                                patch_gapc_task();
                        }
#endif /* (RWBLE_SW_VERSION_MAJOR < 8) */
...
#endif /* ((dg_configBLE_ADV_STOP_DELAY_ENABLE == 1) || (RWBLE_SW_VERSION_MAJOR < 8)) */

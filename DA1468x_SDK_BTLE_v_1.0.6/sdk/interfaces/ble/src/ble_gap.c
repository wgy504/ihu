...
{
        irb_ble_gap_skip_latency_cmd_t *cmd;
        irb_ble_gap_skip_latency_rsp_t *rsp;
        ble_error_t ret;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_GAP_SKIP_LATENCY_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;
        cmd->enable = enable;
        if (!irb_execute(cmd, (void **) &rsp)) {
                return BLE_ERROR_FAILED;
...
{
        irb_ble_gap_data_length_set_cmd_t *cmd;
        irb_ble_gap_data_length_set_rsp_t *rsp;
        ble_error_t ret = BLE_ERROR_FAILED;

        /* setup IRB with new message and fill it */
        cmd = alloc_ble_msg(IRB_BLE_GAP_DATA_LENGTH_SET_CMD, sizeof(*cmd));
        cmd->conn_idx = conn_idx;

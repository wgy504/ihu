...
static void load_part_apv(void)
{
        ...
        for (;;) {
                ...
                case APV_TYPE_BUFFER:
                {
                        ...
                        app_value_put(dev, key, len, ptr, NULL, true);
...
static void dump_device_key_func(const device_t *dev, void *ud)
{
        uint32_t *index = ud;
        /*
...
static void dump_device_apv_func(const device_t *dev, void *ud)
{
        uint32_t *addr_p = ud;
        uint32_t addr;
        uint8_t apv_type;

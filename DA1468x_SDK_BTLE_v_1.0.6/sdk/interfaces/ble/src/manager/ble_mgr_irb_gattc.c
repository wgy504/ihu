 ...
 ****************************************************************************************
 *
 * @file ble_mgr_irb_gattc.c
 *
 * @brief BLE IRB handlers (for GATTC)
 *
...
/** Convert RW UUID to Dialog UUID */
...
/** Convert Dialog UUID to RW UUID */
...
{
        // for IRB
        const irb_ble_gattc_browse_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_BROWSE_CMD, sizeof(*rsp));
        rsp->status = ret;
...
{
        // for IRB
        const irb_ble_gattc_discover_svc_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_DISCOVER_SVC_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_discover_include_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_DISCOVER_INCLUDE_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_discover_char_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_DISCOVER_CHAR_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_discover_desc_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_DISCOVER_DESC_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_read_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_READ_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_write_generic_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_WRITE_GENERIC_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_write_execute_cmd_t *cmd = (void *) irb->ptr_buf;
        irb_ble_gattc_write_execute_rsp_t *rsp;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_WRITE_EXECUTE_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gattc_exchange_mtu_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTC_EXCHANGE_MTU_CMD, sizeof(*rsp));
        rsp->status = ret;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
void irb_ble_handler_gattc_mtu_changed_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_MTU_CHANGED, sizeof(*evt));
...
void irb_ble_handler_gattc_sdp_svc_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        for (i = 0; i < gevt->end_hdl - gevt->start_hdl; i++) {
                ...
                // value is not a separate item - it's part of characteristic item
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_BROWSE_SVC, sizeof(*evt)
        ...
        for (i = 0; i < gevt->end_hdl - gevt->start_hdl; i++) {
                ...
                case GATTC_SDP_ATT_CHAR:
                        ...
                        /*
                         * now need to move to value attribure to retrieve UUID from there
...
void irb_ble_handler_gattc_cmp__browse_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_BROWSE_COMPLETED, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
...
void irb_ble_handler_gattc_disc_svc_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_DISCOVER_SVC, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->start_h = gevt->start_hdl;
        evt->end_h = gevt->end_hdl;
        uuid_rw2dg(gevt->uuid_len, gevt->uuid, &evt->uuid);
...
void irb_ble_handler_gattc_disc_svc_incl_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_DISCOVER_INCLUDE, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
...
void irb_ble_handler_gattc_disc_char_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_DISCOVER_CHAR, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        uuid_rw2dg(gevt->uuid_len, gevt->uuid, &evt->uuid);
        evt->handle = gevt->attr_hdl;
...
void irb_ble_handler_gattc_disc_char_desc_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_DISCOVER_DESC, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        uuid_rw2dg(gevt->uuid_len, gevt->uuid, &evt->uuid);
        evt->handle = gevt->attr_hdl;
...
void irb_ble_handler_gattc_cmp__discovery_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_DISCOVER_COMPLETED, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->status = gevt->status == ATT_ERR_NO_ERROR
                        || gevt->status == ATT_ERR_ATTRIBUTE_NOT_FOUND ? BLE_STATUS_OK : BLE_ERROR_FAILED;
...
void irb_ble_handler_gattc_read_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_READ_COMPLETED, sizeof(*evt) + gevt->length);
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
...
void irb_ble_handler_gattc_cmp__read_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_READ_COMPLETED, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
...
void irb_ble_handler_gattc_cmp__write_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_WRITE_COMPLETED, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->seq_num;
        evt->status = gevt->status; // ATT error
...
void irb_ble_handler_gattc_event_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_NOTIFICATION, sizeof(*evt) + gevt->length);
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->handle;
        evt->length = gevt->length;
        memcpy(evt->value, gevt->value, gevt->length);
...
void irb_ble_handler_gattc_event_req_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTC_INDICATION, sizeof(*evt) + gevt->length);
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->handle;
        evt->length = gevt->length;
        memcpy(evt->value, gevt->value, gevt->length);
...
void irb_ble_handler_gattc_svc_changed_cfg_ind_evt(ble_gtl_msg_t *gtl)
{
        ...
        /*
         * Put this to persistent storage - will be retrieved upon reconnection.
         */

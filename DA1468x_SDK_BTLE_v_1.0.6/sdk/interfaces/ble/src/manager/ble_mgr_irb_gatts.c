 ...
 ****************************************************************************************
 *
 * @file ble_mgr_irb_gatts.c
 *
 * @brief BLE IRB handlers (for GATTS)
 *
...
/** map API properties to RW permissions */
...
/** map API permissions to RW permissions */
...
/** map RW permissions to API properties */
...
/** map RW permissions to API permissions */
...
PRIVILEGED_DATA static irb_ble_stack_msg_t *gattm_add_svc_msg = NULL;
...
{
        // for IRB
        const irb_ble_gatts_service_add_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        gattm_add_svc_msg = ble_gtl_alloc(GATTM_ADD_SVC_REQ, TASK_ID_GATTM, sizeof(struct gattm_add_svc_req) + sizeof(struct gattm_att_desc) * cmd->num_attrs);
        attr_idx = 0;
...
#else
                        (cmd->uuid.type == ATT_UUID_128 ? (2 << 4) : 0) |    // uuid128 vs uuid16
                        (cmd->type == GATT_SERVICE_PRIMARY ? (1 << 6) : 0);  // primary vs secondary
#endif
        ...
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_ADD_CMD, sizeof(*rsp));
        rsp->status = ret;
...
void irb_ble_handler_gatts_service_add_include_cmd(OS_IRB *irb)
{
        ...
        // for IRB
        const irb_ble_gatts_service_add_include_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_INCLUDE_ADD_CMD, sizeof(*rsp));
        rsp->status = ret;
...
void irb_ble_handler_gatts_service_add_characteristic_cmd(OS_IRB *irb)
{
        ...
        // for IRB
        const irb_ble_gatts_service_add_characteristic_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        /* for characteristics, RW permissions are combination of properties and permissions */
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_CHARACTERISTIC_ADD_CMD, sizeof(*rsp));
        rsp->status = ret;
        rsp->h_offset = h_offset;
...
{
        // for IRB
        const irb_ble_gatts_service_add_descriptor_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_DESCRIPTOR_ADD_CMD, sizeof(*rsp));
        rsp->status = ret;
        rsp->h_offset = h_offset;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
void irb_ble_handler_gatts_service_register_cmd(OS_IRB *irb)
{
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_REGISTER_CMD, sizeof(*rsp));

        if (gattm_add_svc_msg) {
                ble_gtl_waitqueue_add(0, GATTM_ADD_SVC_RSP, 0, gatts_service_register_rsp, irb);
...
static void service_enable_reply(OS_IRB *irb, ble_status_t status)
{
        irb_ble_hdr_t *hdr = (void *) irb->ptr_buf;

        irb_ble_mark_completed(irb, true);

        if (hdr->op_code == IRB_BLE_GATTS_SERVICE_ENABLE_CMD) {
                irb_ble_gatts_service_enable_rsp_t *rsp;

                rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_ENABLE_CMD, sizeof(*rsp));
                rsp->status = status;
        } else {
                irb_ble_gatts_service_disable_rsp_t *rsp;

                rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_DISABLE_CMD, sizeof(*rsp));
                rsp->status = status;
        }
...
static void gatts_svc_set_permission_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ...
        service_enable_reply((OS_IRB *) param, grsp->status == 0 ? BLE_STATUS_OK : BLE_ERROR_FAILED);
}
...
static void gatts_svc_get_permission_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ...
        if (grsp->status != 0) {
                service_enable_reply(irb, BLE_ERROR_FAILED);
                return;
        }
        ...
        ble_gtl_waitqueue_add(0, GATTM_SVC_SET_PERMISSION_RSP, 0, gatts_svc_set_permission_rsp, irb);
        ble_gtl_send(gmsg);
}
...
{
        // for IRB
        const irb_ble_gatts_service_enable_cmd_t *cmd = (void *) irb->ptr_buf;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        // don't touch IRB now - it will be used when creating reply

        ble_gtl_waitqueue_add(0, GATTM_SVC_GET_PERMISSION_RSP, 0, gatts_svc_get_permission_rsp, irb);
        ble_gtl_send(gmsg);
}
...
{
        // for IRB
        const irb_ble_gatts_service_disable_cmd_t *cmd = (void *) irb->ptr_buf;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        struct gattm_svc_get_permission_req *gcmd;
...
{
        // for IRB
        const irb_ble_gatts_service_characteristic_get_prop_cmd_t *cmd = (void *) irb->ptr_buf;
        irb_ble_gatts_service_characteristic_get_prop_rsp_t *rsp;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        /*
         * API takes characteristic handle, but for RW we need to read permissions of attribute
         * value handle.
         */
        gcmd->handle = cmd->handle + 1;

        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_CHARACTERISTIC_GET_PROP_CMD, sizeof(*rsp));

        ble_gtl_waitqueue_add(0, GATTM_ATT_GET_PERMISSION_RSP, 0, gatts_att_get_permission_rsp, irb);
        ble_gtl_send(gmsg);
}
...
{
        // for IRB
        const irb_ble_gatts_service_characteristic_set_prop_cmd_t *cmd = (void *) irb->ptr_buf;
        irb_ble_gatts_service_characteristic_set_prop_rsp_t *rsp;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_CHARACTERISTIC_SET_PROP_CMD, sizeof(*rsp));

        ble_gtl_waitqueue_add(0, GATTM_ATT_SET_PERMISSION_RSP, 0, gatts_att_set_permission_rsp, irb);
        ble_gtl_send(gmsg);
}
...
static void gatts_get_value_rsp(ble_gtl_msg_t *gtl, void *param)
{
        ...
        OS_IRB *irb = param;
        irb_ble_gatts_get_value_cmd_t *cmd = irb->ptr_buf;
        irb_ble_gatts_get_value_rsp_t *rsp;
        uint16_t length;

        length = MIN(cmd->max_len, grsp->length);

        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_GET_VALUE_CMD, sizeof(*rsp) + length);
...
{
        // for IRB
        const irb_ble_gatts_get_value_cmd_t *cmd = (void *) irb->ptr_buf;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        // response message will be allocated in gatts_get_value_rsp since its length depends on GTL response

        ble_gtl_waitqueue_add(0, GATTM_ATT_GET_VALUE_RSP, 0, gatts_get_value_rsp, irb);
        ble_gtl_send(gmsg);
}
...
{
        // for IRB
        const irb_ble_gatts_set_value_cmd_t *cmd = (void *) irb->ptr_buf;
        irb_ble_gatts_set_value_rsp_t *rsp;
        // for GTL
        static irb_ble_stack_msg_t *gmsg;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SET_VALUE_CMD, sizeof(*rsp));

        ble_gtl_waitqueue_add(0, GATTM_ATT_SET_VALUE_RSP, 0, gatts_set_value_rsp, irb);
        ble_gtl_send(gmsg);
}
...
{
        // for IRB
        const irb_ble_gatts_read_cfm_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        // GATTC_READ_CFM does not return anything so just complete IRB and send back

        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_READ_CFM_CMD, sizeof(*rsp));
...
{
        // for IRB
        const irb_ble_gatts_write_cfm_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_WRITE_CFM_CMD, sizeof(*rsp));
        rsp->status = BLE_STATUS_OK;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gatts_prepare_write_cfm_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        // GATTC_ATT_INFO_CFM does not return anything so just complete IRB and send back

        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_PREPARE_WRITE_CFM_CMD, sizeof(*rsp));
        rsp->status = BLE_STATUS_OK;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gatts_send_event_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        // Don not wait for GATTC_CMP_EVT, it will be handled async to avoid infinite wait

        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SEND_EVENT_CMD, sizeof(*rsp));
        rsp->status = BLE_STATUS_OK;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
{
        // for IRB
        const irb_ble_gatts_service_changed_ind_cmd_t *cmd = (void *) irb->ptr_buf;
        ...
        irb_ble_mark_completed(irb, true);
        rsp = irb_ble_replace_msg(irb, IRB_BLE_GATTS_SERVICE_CHANGED_IND_CMD, sizeof(*rsp));
        rsp->status = BLE_STATUS_OK;
        ble_mgr_response_queue_send(irb, OS_QUEUE_FOREVER);
}
...
void irb_ble_handler_gatts_read_value_req_evt(ble_gtl_msg_t *gtl)
{
        ...
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTS_READ_REQ, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->handle;
        evt->offset = 0; // RW always request full value
...
void irb_ble_handler_gatts_write_value_req_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTS_WRITE_REQ, sizeof(*evt) + gevt->length);
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->handle;
...
void irb_ble_handler_gatts_prepare_write_req_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTS_PREPARE_WRITE_REQ, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        evt->handle = gevt->handle;
...
void irb_ble_handler_gatts_event_sent_evt(ble_gtl_msg_t *gtl)
{
        ...
        /* setup IRB with new event and fill it */
        evt = irb_ble_init_evt(&irb, BLE_EVT_GATTS_EVENT_SENT, sizeof(*evt));
        evt->conn_idx = TASK_2_CONNIDX(gtl->src_id);
        /* We used sequence number in irb_ble_handler_gatts_send_event_cmd() to store info about handle */

...
void ble_adapter_cmp_evt_init(ad_ble_msg_t *ad_msg, void *param)
{
        ...
        memcpy(devp->irk.key, ad_ble_get_public_address(), BD_ADDR_LEN);    // ensure IRK is unique
        ble_mgr_dev_params_release();
...
static void gapm_reset_complete(ble_gtl_msg_t *gtl, void *param)
{
        ...
        /* Update own public BD address with the one stored in NVPARAM. */
        memcpy(dev_params->own_addr.addr, ad_ble_get_public_address(),
                sizeof(dev_params->own_addr.addr));
        memcpy(dev_params->irk.key, ad_ble_get_public_address(), BD_ADDR_LEN);    // ensure IRK is unique
        dev_params->status = BLE_IS_ENABLED;
        ble_mgr_dev_params_release();

...
/* add new channel with unique Source CID */
...
void irb_ble_handler_l2cap_listen_cmd(OS_IRB *irb)
{
         ...
         * since stack does not allow to manage two channels with the same PSM in reliable way.
         */
...
void irb_ble_handler_l2cap_connect_cmd(OS_IRB *irb)
{
        ...
        /*
         * Do not allow channel connection if another one is being connected on the same link.
        ...
        /*
         * Do not allow channel connection if another one is created or connected on the same PSM
         * since stack does not allow to manage two channels with the same PSM in reliable way.
         */
        chan = find_chan_by_psm(cmd->conn_idx, cmd->psm);
        if (chan) {
...
void irb_ble_handler_l2cap_add_credits_cmd(OS_IRB *irb)
{
        ...
        chan->local_credits += cmd->credits;
...
void irb_ble_handler_l2cap_send_cmd(OS_IRB *irb)
{
        ...
        /*
         * Calculate length of GTL message. We cannot simply use sizeof() since structure has
         * union as one of its members and it does not give accurate value (allocated memory would
         * be larger that what we need and we don't want to waste memory) - so just add size of all
         * fields in relevant part of struct.
         */
...
void irb_ble_handler_gapc_cmp__le_cb_connection_evt(ble_gtl_msg_t *gtl)
{
        ...
        if (gevt->status == GAP_ERR_NO_ERROR) {
                /*
                 * Ignore this since there will be connection indication and we will send (or have
                 * already sent) EVT_L2CAP_CONNECTED
                 */
                return;
        }

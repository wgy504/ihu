...
PRIVILEGED_DATA static uint32_t rtc_time;               // counts in LP cycles
...
__RETAINED_CODE uint32_t rtc_get_fromCPM(uint32_t *lp_prescaled_time, uint32_t *lp_current_time)
{
        HW_TIMER1_GET_INSTANT(*lp_prescaled_time, *lp_current_time);
        rtc_time += (*lp_current_time - rtc_previous_time) & LP_CNT_NATIVE_MASK;
        rtc_previous_time = *lp_current_time;

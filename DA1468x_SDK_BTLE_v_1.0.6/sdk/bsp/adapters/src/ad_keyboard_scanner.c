...
static void activate(void)
{
        ...
        /* Start scanner, don't block sleep for inactivity time period */
        enable_scanner(true);
}
...
static void kbscn_pm_xtal16m_ready_ind_cb(void)
{
        int i;
...
static const adapter_call_backs_t kbscn_pm_callbacks = {
        .ad_xtal16m_ready_ind = kbscn_pm_xtal16m_ready_ind_cb,
...
bool ad_kbscn_init(const ad_kbscn_config *config)
{
        ...
        /* Set inactivity to be greater than user selected debounce time */
        init_env->inactive_time = config->debounce_press_time + 1;

...
static void usage(const char* my_name)
{
	fprintf(stderr,
		...
		"\n"
		"Usage case #1:\n"
		"  %s single in_file version_file out_file [v1.1] [enc [key iv]]\n"
		"\n"
		...
		"  written to 'out_file'.\n"
		"  When v1.1 option is used image for SUOTA 1.1 will be generated.\n"
...
static inline void store_crc_1_1(struct image_header_1_1* hdr, uint32_t crc)
{
	uint32_t val = crc;
	store32(hdr->CRC, val);
}
...
static void write_img_version_1_1(struct image_header_1_1* hdr, const char* s)
{
	/* skip any leading v_ */
	if ('v' == s[0]  &&  '_' == s[1]) {
		s += 2;
...
static void write_img_date_1_1(struct image_header_1_1* hdr, const char* s)
{
	struct tm tm;
	time_t secs;
	...
	strptime(s, "%Y-%m-%d %H:%M", &tm);
#else
	sscanf(s, "%d-%d-%d %d:%d", &tm.tm_year, &tm.tm_mon, &tm.tm_mday, &tm.tm_hour, &tm.tm_min);
	tm.tm_year -= 1900;
	tm.tm_mon--;
#endif
	secs = mktime(&tm);
 ...
 */
static int parse_version_file(FILE* f, void *header, SUOTA_VERSION version)
{
	enum patterns {
		IMG_VERSION = 1,
		IMG_DATE = 2
	...
	do {
		...
		if (patterns_left & IMG_VERSION) {
			from = strstr(line, "BLACKORCA_SW_VERSION ");
			if (from) {
				s = find_cstring(from);
				if (s)
					patterns_left &= ~IMG_VERSION;
				if (version == SUOTA_VERSION_1_1) {
					write_img_version_1_1(header, s);
				} else {
					write_img_version(header, s);
				}
				free(s);
			}
		}
		if (patterns_left & IMG_DATE) {
			from = strstr(line, "BLACKORCA_SW_VERSION_DATE ");
			if (from) {
				s = find_cstring(from);
				if (s)
					patterns_left &= ~IMG_DATE;
				if (version == SUOTA_VERSION_1_1) {
					write_img_date_1_1(header, s);
				} else {
					write_img_date(header, s);
				}
				free(s);
			}
		}
...
static int create_single_image(int argc, const char* argv[])
{
	...
	struct stat sbuf;
	struct image_header hdr;
	struct image_header_1_1 hdr_1_1;
	int encrypt = 0;
	SUOTA_VERSION version = SUOTA_VERSION_1_0;
	void *header = &hdr;
	size_t header_size = sizeof(hdr);
	off_t size;
	int argix = 5;

	if (argc > argix && !strcmp(argv[argix], "v1.1")) {
		// Use new suota version
		version = SUOTA_VERSION_1_1;
		header = &hdr_1_1;
		header_size = sizeof(hdr_1_1);
		argix++;
	}
	...
	if (argc > argix) {
		...
		if (!strcmp(argv[argix], "enc")) {
			...
			if (argc == argix) {
				...
				if (parse_hex_string(argv[argix + 1], user_key,
						sizeof(user_key))) {
					fprintf(stderr, "Invalid key\n");
					usage(argv[0]);
					return EXIT_FAILURE;
				}
				if (parse_hex_string(argv[argix + 2], user_iv,
						sizeof(user_iv))) {
	...
	if (version == SUOTA_VERSION_1_1) {
		memset(&hdr_1_1, 0xff, sizeof(hdr_1_1));
		hdr_1_1.signature[0] = 0x70;
		hdr_1_1.signature[1] = 0x61;
		hdr_1_1.flags = ~0x0000;
		store32(hdr_1_1.exec_location, sizeof(hdr_1_1));
	} else {
	...
	if (version == SUOTA_VERSION_1_1) {
		store32(hdr_1_1.code_size, size);
	} else {
	...
	n = parse_version_file(verf, header, version);
	if (n < 0) {
	...
	if (safe_write(outf, header, header_size)) {
		perror(argv[4]);
		goto cleanup_and_exit;
	}

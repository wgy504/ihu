...
FlagStatus RNG_GetFlagStatus(uint8_t RNG_FLAG)
{
  ...
  if ((RNG_REG->SR & RNG_FLAG) != (uint8_t)RESET)
  {
    /* RNG_FLAG is set */
    bitstatus = SET;
  }
...
void RNG_ClearFlag(uint8_t RNG_FLAG)
{
  ...
  RNG_REG->SR = ~(uint32_t)(((uint32_t)RNG_FLAG) << 4);
}
...
ITStatus RNG_GetITStatus(uint8_t RNG_IT)
{
  ...
  if ((RNG_REG->SR & RNG_IT) != (uint8_t)RESET)
  {
    /* RNG_IT is set */
    bitstatus = SET;
  }

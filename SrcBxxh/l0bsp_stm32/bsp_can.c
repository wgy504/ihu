...
void HAL_CAN_RxCpltCallback(CAN_HandleTypeDef *CanHandle)
{
  ...
  if(CanHandle==&BSP_STM32_CAN_IAU)
  {
		...
		if (BSP_STM32_CAN_IAU_R_State == IHU_HUISTD_RX_STATE_IDLE)
		{
			//只有满足这么苛刻的条件，才算找到了帧头
			if ((res == IHU_HUISTD_FRAME_START_CHAR) && (BSP_STM32_CAN_IAU_R_Count == 1))
			BSP_STM32_CAN_IAU_R_State = IHU_HUISTD_RX_STATE_START;
		}
		else
		{
			//收到CHECKSUM
			if((BSP_STM32_CAN_IAU_R_State == IHU_HUISTD_RX_STATE_START) && (BSP_STM32_CAN_IAU_R_Count == 2))
			{
				BSP_STM32_CAN_IAU_R_State = IHU_HUISTD_RX_STATE_HEADER_CKSM;
			}
			//收到长度高位
			else if((BSP_STM32_CAN_IAU_R_State == IHU_HUISTD_RX_STATE_HEADER_CKSM) && (BSP_STM32_CAN_IAU_R_Count == 3))
			{
				BSP_STM32_CAN_IAU_R_State = IHU_HUISTD_RX_STATE_HEADER_LEN;
			}
			//收到长度低位
			else if((BSP_STM32_CAN_IAU_R_State == IHU_HUISTD_RX_STATE_HEADER_LEN) && (BSP_STM32_CAN_IAU_R_Count == 4))
			{
				...
				BSP_STM32_CAN_IAU_R_State = IHU_HUISTD_RX_STATE_BODY;
			}
			//收到BODY位
			else if((BSP_STM32_CAN_IAU_R_State == IHU_HUISTD_RX_STATE_BODY) && (BSP_STM32_CAN_IAU_R_Len > 1))
			{
			...
			{
				BSP_STM32_CAN_IAU_R_State = IHU_HUISTD_RX_STATE_IDLE;
				BSP_STM32_CAN_IAU_R_Len = 0;
				BSP_STM32_CAN_IAU_R_Count = 0;
				//发送数据到上层CANARIES模块

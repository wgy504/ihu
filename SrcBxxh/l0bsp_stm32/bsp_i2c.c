...
int8_t BSP_STM32_I2C_IAU_R_Buff[BSP_STM32_I2C_IAU_REC_MAXLEN];			//数据接收缓冲区 
int8_t BSP_STM32_I2C_IAU_R_State=0;																	//接收状态
int16_t BSP_STM32_I2C_IAU_R_Count=0;																//当前接收数据的字节数
int16_t BSP_STM32_I2C_IAU_R_Len=0;
int8_t BSP_STM32_I2C_SPARE1_R_Buff[BSP_STM32_I2C_SPARE1_REC_MAXLEN];	//数据接收缓冲区 
...
int ihu_bsp_stm32_i2c_slave_hw_init(void)
{
  ...
  BSP_STM32_I2C_IAU_R_Count = 0;               //接收字符串的起始存储位置
	...
	for(k=0;k<BSP_STM32_I2C_SPARE1_REC_MAXLEN;k++)      //将缓存内容清零
	{
...
void HAL_I2C_RxCpltCallback(I2C_HandleTypeDef *I2cHandle)
{
  ...
  if(I2cHandle==&BSP_STM32_I2C_IAU)
  {
		res = zIhuI2cRxBuffer[BSP_STM32_I2C_IAU_ID-1];
		BSP_STM32_I2C_IAU_R_Buff[BSP_STM32_I2C_IAU_R_Count++] = res;
		if (BSP_STM32_I2C_IAU_R_Count >= BSP_STM32_I2C_IAU_REC_MAXLEN)
			BSP_STM32_I2C_IAU_R_Count = 0;

		//为了IDLE状态下提高效率，直接分解为IDLE和ELSE
		if (BSP_STM32_I2C_IAU_R_State == IHU_HUISTD_RX_STATE_IDLE)
		{
			//只有满足这么苛刻的条件，才算找到了帧头
			if ((res == IHU_HUISTD_FRAME_START_CHAR) && (BSP_STM32_I2C_IAU_R_Count == 1))
			BSP_STM32_I2C_IAU_R_State = IHU_HUISTD_RX_STATE_START;
		}
		else
		{
			//收到CHECKSUM
			if((BSP_STM32_I2C_IAU_R_State == IHU_HUISTD_RX_STATE_START) && (BSP_STM32_I2C_IAU_R_Count == 2))
			{
				BSP_STM32_I2C_IAU_R_State = IHU_HUISTD_RX_STATE_HEADER_CKSM;
			}
			//收到长度高位
			else if((BSP_STM32_I2C_IAU_R_State == IHU_HUISTD_RX_STATE_HEADER_CKSM) && (BSP_STM32_I2C_IAU_R_Count == 3))
			{
				BSP_STM32_I2C_IAU_R_State = IHU_HUISTD_RX_STATE_HEADER_LEN;
			}
			//收到长度低位
			else if((BSP_STM32_I2C_IAU_R_State == IHU_HUISTD_RX_STATE_HEADER_LEN) && (BSP_STM32_I2C_IAU_R_Count == 4))
			{
				...
				if ((BSP_STM32_I2C_IAU_R_Buff[1] == (BSP_STM32_I2C_IAU_R_Buff[0] ^ BSP_STM32_I2C_IAU_R_Buff[2]^BSP_STM32_I2C_IAU_R_Buff[3])) &&\
					(BSP_STM32_I2C_IAU_R_Len < BSP_STM32_I2C_IAU_REC_MAXLEN-4))
				BSP_STM32_I2C_IAU_R_State = IHU_HUISTD_RX_STATE_BODY;
			}
			//收到BODY位
			else if((BSP_STM32_I2C_IAU_R_State == IHU_HUISTD_RX_STATE_BODY) && (BSP_STM32_I2C_IAU_R_Len > 1))
			{
			...
			{
				BSP_STM32_I2C_IAU_R_State = IHU_HUISTD_RX_STATE_IDLE;
				BSP_STM32_I2C_IAU_R_Len = 0;
				BSP_STM32_I2C_IAU_R_Count = 0;
				//发送数据到上层I2CARIES模块
		...
		//重新设置中断
		HAL_I2C_Slave_Receive_IT(&BSP_STM32_I2C_IAU, &zIhuI2cRxBuffer[BSP_STM32_I2C_IAU_ID-1], 1);
  }
//  else if(I2cHandle==&BSP_STM32_I2C_SPARE1)
//  {
//		BSP_STM32_I2C_SPARE1_R_Buff[BSP_STM32_I2C_SPARE1_R_Count] = zIhuI2cRxBuffer[BSP_STM32_I2C_SPARE1_ID-1];
//		BSP_STM32_I2C_SPARE1_R_Count++;
//		if (BSP_STM32_I2C_SPARE1_R_Count >= BSP_STM32_I2C_SPARE1_REC_MAXLEN)
//			BSP_STM32_I2C_SPARE1_R_Count = 0;
//		HAL_I2C_Slave_Receive_IT(&BSP_STM32_I2C_SPARE1, &zIhuI2cRxBuffer[BSP_STM32_I2C_SPARE1_ID-1], 1);
//  }
}

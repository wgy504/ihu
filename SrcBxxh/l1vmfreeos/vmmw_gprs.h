...
//向上提供全局统一服务的入口
extern OPSTAT ihu_vmmw_gprsmod_gsm_all_working_selection(uint8_t option, uint8_t sub_opt);
extern OPSTAT ihu_vmmw_gprsmod_gsm_info_retrieve(void);//SIM800A信息显示(信号质量,电池电量,日期时间)
extern OPSTAT ihu_vmmw_gprsmod_module_info_retrieve(void);//模块信息检测
extern OPSTAT ihu_vmmw_gprsmod_call_perform(uint8_t sub_opt);//拨号测试程序
extern OPSTAT ihu_vmmw_gprsmod_sms_transmit_with_confirm(void); //短信工作流程
extern OPSTAT ihu_vmmw_gprsmod_tcp_data_transmit_with_receive(uint8_t sub_opt);//GPRS数据传输测试程序

=================================================
== 将修改LOG INFO放在最前面，以提高编辑效率
=================================================

**************CCL接下来待完成主要内容**********************************************
=> 后续还需要继续验证 1. 集成测试和稳定性测试 2. 集成IAP，修改SYSDIM为批量生产模式 3. 低功耗优化 4. USB HOST的摄像头方案  5. 有源RFID NRF24L01  
**************************************************************************
...
= 3. 串口摄像头 
= 串口摄像头驱动完成
= CCL全局CONTEXt增加了 UINT8  picBuf[IHU_CCL_PICTURE_BUFFER_SIZE_MAX]，10KB数据区，用来存储图像文件
= 摄像头，暂时只支持一个摄像头的配置情况，多个摄像头的支持，需要增加内部交互控制流
...
= 开始优化整个状态机的工作
= 先要切断/删掉外部中断源的工作逻辑，这部分涉及到独立的中断源接口，目前采用了RTC ALARM闹铃唤醒模式，所以暂时并不需要支持
= 将DIDO_CCL_LOCK_C_DOOR_C_EVENT修改为DIDO_CCL_DOOR_CLOSE_EVENT，因为LOCK信息已经不需要了
= 有关启动源和状态机的设计上，出现了需要硬件改动的需求
	在下位机启动的时候，又遇到一点尴尬的事情：之前我的设计逻辑是，定时8小时汇报和业务状态是完全分离的，两者不相干。
	那这是在CPU还保留上下文的基础下，是有条件如此做，因为所有的MEMEORY数据状态都存在
	现在改为了硬关断，所以每一次系统启动，从应用软件的角度，其实是不知道启动源的
	目前的启动源有三个：8小时正常闹铃、10分钟差错闹铃、人工触发。这在启动的时候，CPU基本上没有啥先验知识。
	如果是这样，我只能将周期性汇报这个状态机和业务状态机进行融合，规定每一次CPU启动，都会直接汇报一次所有的传感器状态，所以也就无所谓这是哪一种源了
	对于10分钟差错闹铃和8小时正常闹铃，因为状态已经汇报了，所以我也不再区分，只是机械的判定，在开锁业务逻辑之前，先扫描么门限是否开，如果是开，就判定系统进入差错状态，如果是闭合，则认为正常。
	这里还有个巨大的漏洞：8小时定时到达，和人工触发，CPU在启动的时候是无法判定的。CPU启动，汇报了状态以后，然后就开始读取钥匙硬件信息，并汇报给后台，获取开锁指令。如果启动源是人工触发，非常正确。如果是定时闹铃，那问题就大了，定时汇报过程会自动将门打开，如果此时虚拟钥匙正好做的是打开的话！！！
	这意味着，我们在硬件设计的时候，必须区分，启动源是闹铃，还是人工触发，不然软件无所适从。。。
	@Xia 门按钮开关拉低，是否可以接一路GPIO到CPU，这样启动的时候，CPU先去读取这个管脚的高地，然后判定这是人工触发，还是闹铃触发
	在闹铃触发源的判定上，可以读取闹铃时间长度，从而准确判定到底是哪一种触发源。
= 增加一个GPIO LCOK_ACT_FLAG的GPIO输入引脚，用来做硬件判定，到底是谁激活了CPU：RTC或者人工触发？
= 完善CCL的状态机：SLEEP状态去掉，ACTIVE增加进来
= SYSDIM中修改了消息支持数量，待消息完善后需要进一步优化
...
= RTC搞定，使用了ALARM闹铃方式：网络日历和BOOT去标记还需要进一步完善，没有这些，这个方案依然是不完善的
= ADC确认是输入参考电压VREF没有连接，导致错误，连上之后就解决了

//= ZJL, 2017 May.2, CURRENT_SW_DELIVERY R03.131 =>CCL项目
= Merge with BFSC update for 207VG
= DIDO的CPU 3.3v控制开关逻辑
=> 锁具改模拟电路，DI/DO控制线逻辑+电源控制逻辑换成磁保持机构
=> 电源启动工作，然后待机关机=》硬激活与待机方案
= 测试并验证BEEP
= 将BEEP做成闪烁图案，放在LED模块中，构成独立的执行逻辑体，测试通过
= 增加RTC的驱动工作逻辑
= 优化WALAGAN在OFF情况下的工作逻辑，确保OFF到达：BEEP一旦闪烁停止而没有OFF，将导致烦人的噪声。
= 测试验证了ADC采样电池的工作逻辑
= RTC待验证
...
= 最终确定DHT11的问题：us的延时要设置合理。每一次读取数据的时候，需要先INIT。多次操作之间要放足延迟。

//= ZJL, 2017 Apr.8, CURRENT_SW_DELIVERY R03.128 =>CCL项目
= 删掉DCMI接口，暂时不用
= 按照原理图，调整管脚配置
= SPI2改为SPI1
= 调测BLE模块，发现项目配置中的NVIC中断没有设置，导致接收不到
= 因为多个项目共享的缘故，接收中断中的IF/ELSE需要改为IF/IF，不然某些逻辑执行不到
= BLE在恢复默认配置(ORGL)以后，速率从9600改为了38400，这个从程序设计上，是一个比较大的坑，9600需要在工厂就一次性搞定，不然到了现场，恢复了以后，结果
  发现速率也变了
= 终于在目标板子上，将BLE调试调通

//= ZJL, 2017 Apr.5, CURRENT_SW_DELIVERY R03.127
= Remove 205RG project
= Add 207VG project
= 移植锁要涉及到的文件
/**********************************************************************************
 *  
 *  1. MAIN函数中有3处修改，外加error_handler中的注释
 *  2. I2C.C中有关I2C的出错处理
 *  3. freertos.c中有关VM的挂载
 *
**********************************************************************************/

//= ZJL, 2017 Mar.28, CURRENT_SW_DELIVERY R03.126 =>CCL项目
= 验证并测试了无源RFID MFRC522技术方案，并集成到板子上了
= 注意，SDA针脚是CS管脚，名字不一样而已。因为程序里，对RST和CS管脚进行了操作，这非常关键！
= SPI2下，无源RFID采用了256分频测试通过的，速率非常低下，只有125kbit级别
= SPI2下，有源RFID采用了256分频没有测试通过，换成8分频，速率在3.75Mbps，也没测试通过。按道理，NRF24L01的SPI主频最高为10MHz，不得高于10Mbps的速率

//= ZJL, 2017 Mar.28, CURRENT_SW_DELIVERY R03.125 =>CCL项目
=> 功耗节电，外部中断源EXTI，降频测试
= 使用外部中断源EXTI，测试了STOP MODE和STANDBY MODE，将入口函数挂载到ccl模块的启动部分，系统均可以进入停止模式和STANDBY模式
= 功耗降低只有50%左右，远远未到10uA的理论水平，未来需要进一步优化各个管脚的配置
...
=> RFID的NRF24L01，以及NFC522
= 查阅SPI的初始化问题，核对NRF24L01的工作逻辑
=> RC522 RFID 13.56MHz
= 集成XPH的MFRF522程序到本项目中完成，待使用真实的13.56MHz模块进行验证
= 倾角传感器测试完毕，所以将周期汇报周期从60秒改为600S，未来还需要改为8个小时 = 28800秒
= SPI的NSS设置，就是片选信号，只能采用软件方式，而不能使用硬件方式，这是ST官方的第二个坑
  http://bbs.21ic.com/icview-110401-1-1.html
  有关SPI的CS片选信号，拉高拉低、速率问题：将管腿的工作频率放到中等速率MIDDLE，将各个寄存器读写操作放10us的额外延迟，确保工作时序的可靠性
= SPI2的这个改动，在BFSC项目中，一并修改过来
...
= 对其XPH的测试工程样例，将外设的初始化在CUBEMX工程中分离，这样将保持跟硬石科技样例的一致性
= 同步修改CCL工程和BFSC工程
...
= 将CPU主频降低以后，RC522对应的13.56MHz所使用的SPI居然会死循环，可能跟其时钟的初始化有关。
= 核对了SW420震动模块的读取过程，跟烟感、光敏等读取函数一样，不再读取处理，而采用统一化处理过程，因为这些传感器的读取过程没有任何独特之处。
   => SW02震动传感器
= 完成了HC05 BLE模块的集成测试工作
= 修正了一个小BUG：CCL的打印模块信息被TASKMAX覆盖
= BLE的UART工作在38400KBPS，需要区分不同的模块基础速率，确保他们配置在正确的速率下。这只需要配置一下STM32CubeMX即可。

//= ZJL, 2017 Feb.24, CURRENT_SW_DELIVERY R03.119 =>CCL项目/BFSC
= 更新HUITP到2.2版本
= 将huitp.h同时编译到BFSC项目中来，以便同步和共用相应的接口消息
...
= 清理VM的全局变量定义并归一化
= 两个项目回归，均没有问题，而且连带PWM/ETH两个任务模块均改好了

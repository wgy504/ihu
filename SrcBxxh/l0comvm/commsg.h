...
enum IHU_INTER_TASK_MSG_ID
{
	...
	MSG_BLEAPP_VMDA_DISCONNECT,  //ZJL, add to trigger VMDA SHELL disconnect
	MSG_SPS_VMDA_BLE_DATA_DL_RCV,  //send BLE received data to VMDA task
	...
	//VMDA SHELL
	MSG_ID_VMDA_TIMER_1S_PERIOD,
	MSG_ID_VMDA_ASYLIBRA_BLE_CON,
	MSG_ID_VMDA_ASYLIBRA_BLE_DISC,
	MSG_ID_VMDA_ASYLIBRA_DATA_DL,
...
typedef struct msg_struct_sps_vmda_ble_data_dl_rcv
{
...
}msg_struct_sps_vmda_ble_data_dl_rcv_t;
...
typedef struct msg_struct_bleapp_vmda_init
{
    uint32_t length;
}msg_struct_bleapp_vmda_init_t;
typedef struct msg_struct_bleapp_vmda_disconnect
{
    /// length
    uint32_t length;
}msg_struct_bleapp_vmda_disconnect_t;


/*
 *	
 *  BXXH与底层BLEAPP相关的消息结构体定义
 *
 */
//VMDA消息定义
typedef struct msg_struct_vmda_1s_period_timtout
{
	UINT8 length;
}msg_struct_vmda_1s_period_timtout_t;
...
//VMDA消息定义
typedef struct  msg_struct_vmda_asylibra_ble_con //
{
	UINT8 length;
}msg_struct_vmda_asylibra_ble_con_t;
typedef struct  msg_struct_vmda_asylibra_ble_disc //
{
	UINT8 length;
}msg_struct_vmda_asylibra_ble_disc_t;

#define MSG_BUF_MAX_LENGTH_ASY_DATA_GENERAL MAX_IHU_MSG_BODY_LENGTH-4
typedef struct  msg_struct_vmda_asylibra_data_dl //
{
	UINT8 length;
	INT8 dataBuf[MSG_BUF_MAX_LENGTH_ASY_DATA_GENERAL];
}msg_struct_vmda_asylibra_data_dl_t;
...
typedef struct  msg_struct_vmda_asylibra_data_ul //
{
	UINT8 length;
	INT8 dataBuf[MSG_BUF_MAX_LENGTH_ASY_DATA_GENERAL];
}msg_struct_vmda_asylibra_data_ul_t;

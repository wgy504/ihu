...
OPSTAT fsm_emc68x_task_entry(UINT8 dest_id, UINT8 src_id, void * param_ptr, UINT16 param_len)
{
	...
	if (FsmSetState(TASK_ID_EMC68X, FSM_STATE_IDLE) == FAILURE){
...
//		}
	...
	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_EMC68X, FSM_STATE_EMC68X_INITED) == FAILURE){
		IhuErrorPrint("EMC68X: Error Set FSM State!");	
		return FAILURE;
	}

	//初始化硬件接口
	if (func_emc68x_hw_init() == FAILURE){	
	...
	//设置状态机到目标状态
	if (FsmSetState(TASK_ID_EMC68X, FSM_STATE_EMC68X_AVTIVE) == FAILURE){

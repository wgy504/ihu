...
** FSM of the I2CARIES
*/
IhuFsmStateItem_t IhuFsmI2caries[] =
{
  ...
  {MSG_ID_COM_INIT,       								FSM_STATE_I2CARIES_INITED,            			fsm_i2caries_init},
	{MSG_ID_COM_INIT_FB,       							FSM_STATE_I2CARIES_INITED,            			fsm_com_do_nothing},

	//ANY state entry
  {MSG_ID_COM_INIT_FB,                    FSM_STATE_COMMON,                           fsm_com_do_nothing},
	{MSG_ID_COM_HEART_BEAT,                 FSM_STATE_COMMON,                           fsm_com_heart_beat_rcv},
	{MSG_ID_COM_HEART_BEAT_FB,              FSM_STATE_COMMON,                           fsm_com_do_nothing},
	{MSG_ID_COM_STOP,                       FSM_STATE_COMMON,                           fsm_i2caries_stop_rcv},
  {MSG_ID_COM_RESTART,                    FSM_STATE_COMMON,                           fsm_i2caries_restart},
	{MSG_ID_COM_TIME_OUT,                   FSM_STATE_COMMON,                           fsm_i2caries_time_out},
...
//	zIhuCclI2cariesCtrlTable.cclI2cWorkingMode = IHU_CCL_I2C_WORKING_MODE_SLEEP;  //初始化就进入SLEEP，然后就看是否有触发
	...
	if (IHU_I2CARIES_PERIOD_TIMER_SET == IHU_I2CARIES_PERIOD_TIMER_ACTIVE){
		//测试性启动周期性定时器
		ret = ihu_timer_start(TASK_ID_I2CARIES, TIMER_ID_1S_I2CARIES_PERIOD_SCAN, \
			zIhuSysEngPar.timer.array[TIMER_ID_1S_I2CARIES_PERIOD_SCAN].dur, TIMER_TYPE_PERIOD, TIMER_RESOLUTION_1S);
		if (ret == IHU_FAILURE){
			zIhuSysStaPm.taskRunErrCnt[TASK_ID_I2CARIES]++;
...
//SLEEP&FAULT模式下扫描：扫描rsv1, 数据格式HUITP_IEID_UNI_COM_FORMAT_TYPE_FLOAT_WITH_NF2
INT16 ihu_didocap_ccl_sleep_and_fault_mode_ul_scan_illegal_rsv1_value(void)
{
	return rand()% 1000000;
}

//SLEEP&FAULT模式下扫描：扫描rsv2, 数据格式HUITP_IEID_UNI_COM_FORMAT_TYPE_FLOAT_WITH_NF2
INT16 ihu_didocap_ccl_sleep_and_fault_mode_ul_scan_illegal_rsv2_value(void)
{
	return rand()% 1000000;
}

...
#if ((HUITP_MSG_BUF_WITH_HEAD_MAX_LEN > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX) || (HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX) ||\
		(HUITP_MSG_HUIXML_HEAD_IN_CHAR_VARIABLE_LEN <= 0) || (HUITP_MSG_HUIXML_TOTAL_BUF_IN_CHAR_MAX_LEN > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX) ||\
		(HUITP_MSG_HUIFRAME_TOTAL_BUF_MAX_LEN > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX))
...
OPSTAT func_cloud_standard_xml_pack(UINT8 msgType, char *funcFlag, UINT16 msgId, StrMsg_HUITP_MSGID_uni_general_message_t *inputPar, UINT16 inputLen, CloudDataSendBuf_t *output)
{
	...
	char s[IHU_SYSMSG_BH_BUF_BODY_LEN_MAX - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN];
	
	//参数检查：特别要检查输入的数据长度，正常不能超过100，因为HUIXML的数据区= (500(最长消息长度)-300(XML头))/2=100，这在大多数情况下都够用的，但在
	//文件传输的条件下有可能不够。幸好，文件下载使用FFP模式，不用再担心这个了。
	if ((inputLen <4) || (inputPar == NULL) || (inputLen > (IHU_SYSMSG_BH_BUF_BODY_LEN_MAX - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN)/2) || \
		(inputLen > (sizeof(StrMsg_HUITP_MSGID_uni_general_message_t) - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN)))
	{
		IhuDebugPrint("HUITPXML: InputLen=%d, InputPar=%d, MaxLen=%d, size2=%d\n", inputLen, inputPar, (IHU_SYSMSG_BH_BUF_BODY_LEN_MAX - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN)/2, sizeof(StrMsg_HUITP_MSGID_uni_general_message_t));
  ...
  sprintf(s, "%d", timeStamp);
	...
	if ((strlen(s) < 4) || (strlen(s) > (IHU_SYSMSG_BH_BUF_BODY_LEN_MAX - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN))){
		zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
...
OPSTAT func_cloud_standard_xml_unpack(msg_struct_ccl_com_cloud_data_rx_t *rcv, int expectMsgId)
{
	...
	char msgToUser[IHU_SYSDIM_FILE_NAME_LEN_MAX], msgFromUser[IHU_SYSDIM_FILE_NAME_LEN_MAX], msgFuncFlag[IHU_SYSDIM_FILE_NAME_LEN_MAX];
	char msgTmp[IHU_SYSDIM_FILE_NAME_LEN_MAX];
	...
	if ((rcv->length > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX) || (rcv->length < 8)){
	...
	if ((pIndexT1 == NULL) || (pIndexT2 == NULL) || ((pIndexT1 +strlen(HUITP_MSG_HUIXML_CONSTANT_XML_HEAD_L))>= pIndexT2) || (dif > IHU_SYSMSG_BH_BUF_BODY_LEN_MAX)){
	...
	if ((pIndexT1 == NULL) || (pIndexT2 == NULL) || ((pIndexT1 +strlen(HUITP_MSG_HUIXML_CONSTANT_TO_USER_L))>= pIndexT2) || (dif > IHU_SYSDIM_FILE_NAME_LEN_MAX)){
	...
	if ((pIndexT1 == NULL) || (pIndexT2 == NULL) || ((pIndexT1 +strlen(HUITP_MSG_HUIXML_CONSTANT_FROM_USER_L))>= pIndexT2) || (dif > IHU_SYSDIM_FILE_NAME_LEN_MAX)){
	...
	if ((pIndexT1 == NULL) || (pIndexT2 == NULL) || ((pIndexT1 +strlen(HUITP_MSG_HUIXML_CONSTANT_MSG_TYPE_L))>= pIndexT2) || (dif > IHU_SYSDIM_FILE_NAME_LEN_MAX)){
	...
	if ((pIndexT1 == NULL) || (pIndexT2 == NULL) || ((pIndexT1 +strlen(HUITP_MSG_HUIXML_CONSTANT_FUNC_FLAG_L))>= pIndexT2) || (dif > IHU_SYSDIM_FILE_NAME_LEN_MAX)){
	...
	if (msgLen > (IHU_SYSMSG_BH_BUF_BODY_LEN_MAX - HUITP_MSG_HUIXML_HEAD_IN_CHAR_MAX_LEN)/2){
...
//			pMsgBuf.msgLen = HUITP_ENDIAN_EXG16(sizeof(StrMsg_HUITP_MSGID_uni_sw_package_confirm_t) - 4);
	...
	switch(msgId)
	{
		...
		case HUITP_MSGID_uni_heart_beat_req:
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_heart_beat_req_received_handle(snd);
		}
			break;
		...
		case HUITP_MSGID_uni_heart_beat_confirm:
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_heart_beat_confirm_received_handle(snd);			
		}
			break;
		
		case HUITP_MSGID_uni_ccl_lock_req:	
		{
			StrMsg_HUITP_MSGID_uni_ccl_lock_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_lock_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_lock_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_lock_req_received_handle(snd);	
		}
			break;
			
		case HUITP_MSGID_uni_ccl_lock_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_lock_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_lock_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_lock_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_lock_confirm_received_handle(snd);
		}
			break;
			
		case HUITP_MSGID_uni_ccl_lock_auth_resp:
		{
			StrMsg_HUITP_MSGID_uni_ccl_lock_auth_resp_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_lock_auth_resp_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_lock_auth_resp_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_lock_auth_resp_received_handle(snd);			
		}
			break;
			
		case HUITP_MSGID_uni_ccl_door_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_door_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_door_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_door_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_door_req_received_handle(snd);			
		}
			break;
		
		case HUITP_MSGID_uni_ccl_door_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_door_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_door_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_door_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_door_confirm_received_handle(snd);						
		}
			break;
				
		case HUITP_MSGID_uni_ccl_rfid_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_rfid_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_rfid_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_rfid_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_rfid_req_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_rfid_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_rfid_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_rfid_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_rfid_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_rfid_confirm_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_ble_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_ble_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_ble_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_ble_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_ble_req_received_handle(snd);					
		}
			break;

		case HUITP_MSGID_uni_ccl_ble_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_ble_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_ble_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_ble_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_ble_confirm_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_gprs_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_gprs_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_gprs_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_gprs_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_gprs_req_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_gprs_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_gprs_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_gprs_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_gprs_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_gprs_confirm_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_battery_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_battery_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_battery_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_battery_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_battery_req_received_handle(snd);							
		}
			break;

		case HUITP_MSGID_uni_ccl_battery_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_battery_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_battery_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_battery_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_battery_confirm_received_handle(snd);								
		}
			break;

		case HUITP_MSGID_uni_ccl_shake_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_shake_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_shake_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_shake_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_shake_req_received_handle(snd);					
		}
			break;

		case HUITP_MSGID_uni_ccl_shake_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_shake_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_shake_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_shake_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_shake_confirm_received_handle(snd);							
		}
			break;

		case HUITP_MSGID_uni_ccl_smoke_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_smoke_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_smoke_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_smoke_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_smoke_req_received_handle(snd);								
		}
			break;

		case HUITP_MSGID_uni_ccl_smoke_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_smoke_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_smoke_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_smoke_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_smoke_confirm_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_water_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_water_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_water_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_water_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_water_req_received_handle(snd);				
		}
			break;

		case HUITP_MSGID_uni_ccl_water_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_water_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_water_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_water_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_water_confirm_received_handle(snd);							
		}
			break;

		case HUITP_MSGID_uni_ccl_temp_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_temp_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_temp_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_temp_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_temp_req_received_handle(snd);				
		}
			break;

		case HUITP_MSGID_uni_ccl_temp_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_temp_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_temp_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_temp_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_temp_confirm_received_handle(snd);							
		}
			break;

		case HUITP_MSGID_uni_ccl_humid_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_humid_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_humid_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_humid_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_humid_req_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_humid_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_humid_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_humid_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_humid_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_humid_confirm_received_handle(snd);								
		}
			break;

		case HUITP_MSGID_uni_ccl_fall_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_fall_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_fall_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_fall_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_fall_req_received_handle(snd);						
		}
			break;

		case HUITP_MSGID_uni_ccl_fall_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_fall_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_fall_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_fall_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_fall_confirm_received_handle(snd);
		}
			break;

		case HUITP_MSGID_uni_ccl_state_req:
		{
			StrMsg_HUITP_MSGID_uni_ccl_state_req_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_state_req_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_state_req_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_state_req_received_handle(snd);			
		}
			break;

		case HUITP_MSGID_uni_ccl_state_confirm:
		{
			StrMsg_HUITP_MSGID_uni_ccl_state_confirm_t *snd;
			if (msgLen != (sizeof(StrMsg_HUITP_MSGID_uni_ccl_state_confirm_t) - 4)){
				zIhuSysStaPm.taskRunErrCnt[TASK_ID_SPSVIRGO]++;
				IhuErrorPrint("HUITPXML: Error unpack message on length!\n");
				return IHU_FAILURE;				
			}
			snd = (StrMsg_HUITP_MSGID_uni_ccl_state_confirm_t*)(&pMsgBuf);
			ret = func_cloud_spsvirgo_ccl_msg_ccl_state_confirm_received_handle(snd);						
		}
			break;
		...
		case HUITP_MSGID_uni_inventory_req:			
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_inventory_req_received_handle(snd);			
		}
			break;
		...
		case HUITP_MSGID_uni_inventory_confirm:
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_inventory_confirm_received_handle(snd);					
		}
			break;		
		...
		case HUITP_MSGID_uni_sw_package_req:
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_sw_package_req_received_handle(snd);							
		}
			break;		
		...
		case HUITP_MSGID_uni_sw_package_confirm:
		{
			...
			ret = func_cloud_spsvirgo_ccl_msg_sw_package_confirm_received_handle(snd);						
		}
			break;
...
//	}
	...
	pMsgProc3.dcmiValue.dcmiValue = HUITP_ENDIAN_EXG16(ihu_dcmiaris_ccl_sleep_and_fault_mode_ul_scan_illegal_dcmi_value());	
	//Pack message
	memset(&pMsgInput, 0, sizeof(StrMsg_HUITP_MSGID_uni_general_message_t));

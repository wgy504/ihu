...
OPSTAT fsm_gpiocancer_time_out(UINT8 dest_id, UINT8 src_id, void * param_ptr, UINT16 param_len)
{
	...
	if ((param_ptr == NULL || param_len > sizeof(msg_struct_com_time_out_t))){
		IhuErrorPrint("GPIOCANCER: Receive message error!\n");
		zIhuRunErrCnt[TASK_ID_GPIOCANCER]++;
		return IHU_FAILURE;
	}
	memcpy(&rcv, param_ptr, param_len);

	//钩子在此处，检查zIhuRunErrCnt[TASK_ID_GPIOCANCER]是否超限
	if (zIhuRunErrCnt[TASK_ID_GPIOCANCER] > IHU_RUN_ERROR_LEVEL_2_MAJOR){
		//减少重复RESTART的概率
		zIhuRunErrCnt[TASK_ID_GPIOCANCER] = zIhuRunErrCnt[TASK_ID_GPIOCANCER] - IHU_RUN_ERROR_LEVEL_2_MAJOR;
		memset(&snd0, 0, sizeof(msg_struct_com_restart_t));
		snd0.length = sizeof(msg_struct_com_restart_t);
		ret = ihu_message_send(MSG_ID_COM_RESTART, TASK_ID_GPIOCANCER, TASK_ID_GPIOCANCER, &snd0, snd0.length);
		if (ret == IHU_FAILURE){
			zIhuRunErrCnt[TASK_ID_GPIOCANCER]++;
			IhuErrorPrint("GPIOCANCER: Send message error, TASK [%s] to TASK[%s]!\n", zIhuTaskNameList[TASK_ID_GPIOCANCER], zIhuTaskNameList[TASK_ID_GPIOCANCER]);
			return IHU_FAILURE;
		}
	}

	//Period time out received
	if ((rcv.timeId == TIMER_ID_1S_GPIOCANCER_PERIOD_SCAN) &&(rcv.timeRes == TIMER_RESOLUTION_1S)){
		//保护周期读数的优先级，强制抢占状态，并简化问题
		if (FsmGetState(TASK_ID_GPIOCANCER) != FSM_STATE_GPIOCANCER_ACTIVED){
			ret = FsmSetState(TASK_ID_GPIOCANCER, FSM_STATE_GPIOCANCER_ACTIVED);
			if (ret == IHU_FAILURE){
				zIhuRunErrCnt[TASK_ID_GPIOCANCER]++;
				IhuErrorPrint("GPIOCANCER: Error Set FSM State!\n");
				return IHU_FAILURE;
		...
		func_gpiocancer_time_out_period_scan();
	}
...
{
	IhuDebugPrint("GPIOCANCER: Time Out Test!\n");
}

...
FsmStateItem_t FsmPwmtaurus[] =
{
  ...
  {MSG_ID_COM_RESTART,										FSM_STATE_IDLE,            									fsm_pwmtaurus_restart},

  //Task level initialization
  {MSG_ID_COM_RESTART,        						FSM_STATE_PWMTAURUS_INITED,         					fsm_pwmtaurus_restart},
  {MSG_ID_COM_STOP,												FSM_STATE_PWMTAURUS_INITED,         					fsm_pwmtaurus_stop_rcv},
  ...
  {MSG_ID_COM_STOP,												FSM_STATE_PWMTAURUS_ACTIVED,         					fsm_pwmtaurus_stop_rcv},
	{MSG_ID_COM_TIME_OUT,										FSM_STATE_PWMTAURUS_ACTIVED,         				  fsm_pwmtaurus_time_out},
...
OPSTAT fsm_pwmtaurus_task_entry(UINT8 dest_id, UINT8 src_id, void * param_ptr, UINT16 param_len)
{
	...
	if (FsmSetState(TASK_ID_PWMTAURUS, FSM_STATE_IDLE) == FAILURE){
...
//		}
	...
	//收到初始化消息后，进入初始化状态
	if (FsmSetState(TASK_ID_PWMTAURUS, FSM_STATE_PWMTAURUS_INITED) == FAILURE){
		IhuErrorPrint("PWMTAURUS: Error Set FSM State!\n");	
		return FAILURE;
	}

	//初始化硬件接口
	if (func_pwmtaurus_hw_init() == FAILURE){	
	...
	//设置状态机到目标状态
	if (FsmSetState(TASK_ID_PWMTAURUS, FSM_STATE_PWMTAURUS_ACTIVED) == FAILURE){
...
{
	IhuDebugPrint("PWMTAURUS: Time Out Test!\n");
}
